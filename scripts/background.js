...
chrome.runtime.onMessage.addListener(function (req, sender, sendResponse) {
  ...
  if (req.cmd == "isRefreshing") {
    return sendResponse(
      currentTabs[req.tab.id] == undefined
        ? false
        : currentTabs[req.tab.id].isActive
    );
  }
...
async function updateUIState(tabId) {
  if (!currentTabs[tabId].ui_state)
    currentTabs[tabId].ui_state = {
      badgeText: "",
      icon: "inactive",
    };
  ...
  if (currentTabs[tabId].isActive) {
    let timeLeft = Math.ceil(
      (currentTabs[tabId].nextRefresh - Date.now()) / 1000
    );
    if (currentTabs[tabId].interval < 1000 && timeLeft == 1)
      timeLeft = currentTabs[tabId].interval / 1000;
  ...
  if (
    currentState.badgeText != newState.badgeText &&
    currentState.badgeText != "0"
  ) {
...
async function refreshTab(tabId) {
  ...
  (async () => {
    // this is a fallback in case the callback isn't called (this can be the case if the refresh was manually canceled by a user)
    while (!resolved) {
      const tab = await chrome.tabs.get(tabId);
